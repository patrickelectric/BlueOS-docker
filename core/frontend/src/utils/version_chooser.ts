...
function nestVersions(versions: Version[]): NestedVersion[] {
  ...
  // Several tags can point at the same image, so its children are only expanded under the first of them
...
async function commitVersion(repository: string, tag: string): Promise<void> {
  await back_axios({
    ...
    data: { repository, tag },
    // Exporting the container layer takes as long as the filesystem diff is big
    timeout: 10 * 60 * 1000,
  })
}
