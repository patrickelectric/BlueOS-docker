<template>
  ...
  >
    ...
    <div class="d-flex flex-wrap justify-center align-center my-2">
      ...
      <v-btn
        v-if="showCommit"
        v-tooltip="'Saves the container filesystem as a new image, settings and user data are not included'"
        color="primary"
        class="mx-2 my-1"
        :disabled="working"
...
export default Vue.extend({
  ...
  computed: {
    ...
    showCommit(): boolean {
      return this.settings.is_pirate_mode && this.current && !this.remote
    },
    displayTag(): string {
