  ...
  computed: {
    values():
      ...
      const values = [
        {
          name: 'Position',
          value: `${this.lattitude.toFixed(2)}º / ${this.longitude.toFixed(2)}º`,
        ...
        values.push(
          {
            ...
            tooltip: 'Horizontal Accuracy in meters',
            icon: 'mdi-arrow-expand-horizontal',
          },
          {
            name: 'VACC',
            value: `${(this.gps_raw_int.v_acc / 1000).toFixed(2)} m`,
            tooltip: 'Vertical Accuracy in meters',
    ...
    map_link(): string {
      const zoom = 16
      const marker = `lat=${this.lattitude}&lon=${this.longitude}`
      const map = `${zoom}/${this.lattitude}/${this.longitude}&layers=N`
